...
import (
	...
	nginxvalidation "github.com/nginx/kubernetes-ingress/internal/validation"
...
func validateHTTPHeadersAnnotation(context *annotationValidationContext) field.ErrorList {
	...
	for _, header := range nginxvalidation.SplitHeaderNameList(context.value) {
		for _, msg := range nginxvalidation.ValidateHeaderName(header) {
...
func validateLBMethodAnnotation(context *annotationValidationContext) field.ErrorList {
	...
	if strings.HasPrefix(method, "hash ") {
		if err := nginxvalidation.ValidateDirectiveValue(context.value); err != nil {
			return field.ErrorList{field.Invalid(context.fieldPath, context.value, err.Error())}
		}
...
func validateStickyServiceListAnnotation(context *annotationValidationContext) field.ErrorList {
	...
	for _, parameters := range services {
		if err := nginxvalidation.ValidateDirectiveValue(parameters); err != nil {
			return field.ErrorList{field.Invalid(context.fieldPath, context.value, err.Error())}
		}
...
func validateRewriteListAnnotation(context *annotationValidationContext) field.ErrorList {
	...
	for service, rewrite := range rewrites {
		if err := nginxvalidation.ValidateDirectiveToken(rewrite); err != nil {
			return field.ErrorList{field.Invalid(context.fieldPath, context.value, err.Error())}
		}
...
func validateRewriteTargetAnnotation(context *annotationValidationContext) field.ErrorList {
	...
	if err := nginxvalidation.ValidateDirectiveToken(target); err != nil {
		return field.ErrorList{field.Invalid(context.fieldPath, target, err.Error())}
	}
...
func validateSSLCiphersAnnotation(context *annotationValidationContext) field.ErrorList {
	allErrs := field.ErrorList{}
	if !nginxvalidation.SSLCiphersRegex.MatchString(context.value) {
