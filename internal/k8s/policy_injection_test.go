...
var policyStringFields = []policyField{
	...
	{"externalAuth.authURI", func(s *conf_v1.PolicySpec, v string) {
		s.ExternalAuth = &conf_v1.ExternalAuth{AuthURI: v, AuthServiceName: "auth-svc"}
	}, "/auth", nil},
	{"externalAuth.authSigninURI", func(s *conf_v1.PolicySpec, v string) {
		s.ExternalAuth = &conf_v1.ExternalAuth{AuthURI: "/auth", AuthServiceName: "auth-svc", AuthSigninURI: v}
	}, "/signin", nil},
	{"externalAuth.sniName", func(s *conf_v1.PolicySpec, v string) {
		s.ExternalAuth = &conf_v1.ExternalAuth{AuthURI: "/auth", AuthServiceName: "auth-svc", SNIName: v}
	}, "auth.example.com", nil},
	{"externalAuth.authServiceName", func(s *conf_v1.PolicySpec, v string) {
		s.ExternalAuth = &conf_v1.ExternalAuth{AuthURI: "/auth", AuthServiceName: v}
	}, "auth-svc", nil},
	{"externalAuth.authSigninRedirectBasePath", func(s *conf_v1.PolicySpec, v string) {
		s.ExternalAuth = &conf_v1.ExternalAuth{AuthURI: "/auth", AuthServiceName: "auth-svc", AuthSigninURI: "/signin", AuthSigninRedirectBasePath: v}
	}, "/oauth2", nil},
	{"externalAuth.trustedCertSecret", func(s *conf_v1.PolicySpec, v string) {
		s.ExternalAuth = &conf_v1.ExternalAuth{AuthURI: "/auth", AuthServiceName: "auth-svc", SSLEnabled: true, SSLVerify: true, TrustedCertSecret: v}
	}, "externalauth-ca-secret", nil},
	...
	for _, isPlus := range []bool{false, true} {
		...
		for _, field := range policyStringFields {
			...
			if field.plusOnly() && !isPlus {
				if validation.ValidatePolicy(benign, cfg) == nil {
			...
			if err := validation.ValidatePolicy(benign, cfg); err != nil {
			...
			for _, payload := range payloads {
				...
				if validation.ValidatePolicy(policy, cfg) != nil {
					byValidation++
					continue
				}
				admitted++
