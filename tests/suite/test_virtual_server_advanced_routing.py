...
class TestAdvancedRouting:
    def test_flow_with_header(self, kube_apis, crd_ingress_controller, virtual_server_setup):
        ...
        resp_1 = requests.get(
            virtual_server_setup.backend_1_url, headers={"host": virtual_server_setup.vs_host, "x-version": "future"}
        )
        resp_2 = requests.get(
    ...
    def test_flow_with_cookie(self, kube_apis, crd_ingress_controller, virtual_server_setup):
        ...
        resp_1 = requests.get(
            virtual_server_setup.backend_1_url, headers={"host": virtual_server_setup.vs_host}, cookies={"user": "some"}
        )
        resp_2 = requests.get(
            virtual_server_setup.backend_1_url, headers={"host": virtual_server_setup.vs_host}, cookies={"user": "bad"}
        )
        resp_3 = requests.get(
            virtual_server_setup.backend_1_url,
