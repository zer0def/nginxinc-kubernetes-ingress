...
2. For the NGINX App Protect DoS module, create CRDs for `APDosPolicy`, `APDosLogConf` and `DosProtectedResource`:
    ...
    ```shell
    kubectl apply -f https://raw.githubusercontent.com/nginxinc/kubernetes-ingress/v3.6.1/deploy/crds-nap-dos.yaml
    ```
...
3. **Delete the Custom Resource Definitions**:
   ...
   1. Delete core custom resource definitions:
    ```shell
    kubectl delete -f https://raw.githubusercontent.com/nginxinc/kubernetes-ingress/v3.6.1/deploy/crds.yaml
    ```
