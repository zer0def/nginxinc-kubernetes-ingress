...
2. Use Docker to pull the required image from `private-registry.nginx.com`. Choose the image from the available images listed in the [tech specs guide]({{< relref "technical-specifications#images-with-nginx-plus" >}}).
   ...
   ```
   $ docker pull private-registry.nginx.com/nginx-ic/nginx-plus-ingress:2.2.2
   ```
...
3. Tag and push the image to your private registry.
   ...
   ```
   $ docker tag private-registry.nginx.com/nginx-ic/nginx-plus-ingress:2.2.2 <my-docker-registry>/nginx-ic/nginx-plus-ingress:2.2.2
   $ docker push <my-docker-registry>/nginx-ic/nginx-plus-ingress:2.2.2
   ```

   or for NGINX App Protect enabled image
   ```
   $ docker tag private-registry.nginx.com/nginx-ic-nap/nginx-plus-ingress:2.2.2 <my-docker-registry>/nginx-ic-nap/nginx-plus-ingress:2.2.2
   $ docker push <my-docker-registry>/nginx-ic-nap/nginx-plus-ingress:2.2.2
   ```
